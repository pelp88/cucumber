...
public class ChemicalLevelReadingVisitor implements ReadingVisitor {
    ...
    public void visit(ReadingDto reading, FieldRsDto field) {
        ...
        if (reading.getChemicalLevel() < plant.getChemical()) {
            taskService.create(new TaskRqDto(field.getId(), TaskStatus.OPEN, "Обработать средством от паразитов"));
        }
    }
}
